...
export default function BackupManager() {
  ...
  const { vault, setUnlockedVault, masterPassword, login, kdfParams, kdfSalt } =
    useVault();
  const [isImporting, setIsImporting] = useState(false);
  const [pendingBackup, setPendingBackup] = useState<EncryptedVault | null>(
    null,
  );
  ...
  const handleExport = async () => {
    if (!vault || !masterPassword || !login || !kdfParams || !kdfSalt) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Vault is not loaded.",
      });
      return;
    }
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (
        !backupData.vault_ciphertext ||
        !backupData.vault_nonce ||
        !backupData.kdf_salt ||
        !backupData.kdf_params ||
        backupData.vault_version === undefined
      ) {
  ...
  const finalizeImport = async () => {
    ...
    if (!importLogin) {
      toast({
        ...
        title: "Import Failed",
        description:
          "Login is missing. Please unlock or include login in backup.",
      });
      return;
    }
    if (!importPassword) {
    ...
    try {
      ...
      const importAuthToken = await deriveAuthToken(
        importPassword,
        base64ToArrayBuffer(pendingBackup.kdf_salt),
        pendingBackup.kdf_params,
      );
      ...
      try {
        let response = await fetch("/api/vault", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            login: importLogin,
            ...encrypted,
            auth_token: importAuthToken,
          }),
        });
        ...
        toast({
          ...
          description:
            "Server unreachable. Vault restored locally; sync when online.",
        });
      }

      await persistEncryptedVault(importLogin, encrypted);
      const fullVault: Vault = {
        ...decryptedVault,
        vault_version: encrypted.vault_version,
      };
      setUnlockedVault(
        fullVault,
        importPassword,
        importLogin,
        { salt: encrypted.kdf_salt, params: encrypted.kdf_params },
        importAuthToken,
      );

      toast({
        title: "Import Successful",
        description: "Vault has been restored from backup.",
      });
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <Card className="bg-card/50 backdrop-blur-lg border border-border/30">
          <CardHeader>
            ...
            <CardDescription>
              Download a local, encrypted backup of your vault. Keep this file
              safe.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-card/50 backdrop-blur-lg border border-border/30">
          <CardHeader>
            ...
            <CardDescription>
              Restore your vault from an encrypted backup file. This will
              overwrite your current vault.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      >
        <DialogContent className="sm:max-w-[450px] bg-background/80 backdrop-blur-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              To restore this encrypted backup, enter the master password used
              to create it. It is never sent to the server.
            </DialogDescription>
          </DialogHeader>
